      ...
      <div class="header-top-inner">
        <div class="cnt-account">
          <ul class="list-unstyled">
            <li><a href="#"><i class="icon fa fa-user"></i>{{ (session()->get('language') == 'hindi' )?'मेरा खाता':'My Account'}}</a></li>
            <li><a href="#"><i class="icon fa fa-heart"></i>{{ (session()->get('language') == 'hindi' )?'इच्छा-सूची':'Wishlist'}}</a></li>
            <li><a href="#"><i class="icon fa fa-shopping-cart"></i>{{(session()->get('language') == 'hindi' )?'मेरी गाड़ी':'My Cart'}}</a></li>
            <li><a href="#"><i class="icon fa fa-check"></i>{{ (session()->get('language') == 'hindi' )?'चेक आउट':'Checkout'}}</a></li>
            <li>
                @auth
                <a href="{{ route('dashboard') }}"><i class="icon fa fa-lock"></i>{{(session()->get('language') == 'hindi' )?'प्रोफ़ाइल':'Profile'}}</a>
                @else
                <a href="{{ route('login') }}"><i class="icon fa fa-lock"></i>{{ (session()->get('language') == 'hindi' )?'लॉग इन/रजिस्टर':'Login/Register'}}</a>
                @endauth
            </li>
          </ul>
        </div>
        ...
        <div class="cnt-block">
          <ul class="list-unstyled list-inline">
            <li class="dropdown dropdown-small"> <a href="#" class="dropdown-toggle" data-hover="dropdown" data-toggle="dropdown"><span class="value">{{ (session()->get('language') == 'hindi' )?'हमडी':'USD'}} </span><b class="caret"></b></a>
              <ul class="dropdown-menu">
                <li><a href="#">{{ (session()->get('language') == 'hindi' )?'हमडी':'USD'}}</a></li>
                <li><a href="#">{{ (session()->get('language') == 'hindi' )?'आईएनआर':'INR'}}</a></li>
                <li><a href="#">{{  (session()->get('language') == 'hindi' )?'जीबीपी':'GBP' }}</a></li>
              </ul>
            </li>
    ...
    <div class="container">
      <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-3 logo-holder">
          <!-- ============================================================= LOGO ============================================================= -->
          <div class="logo"> <a href="{{ route('index') }}"> <img src="{{ asset('frontend_assets') }}/assets/images/logo.png" alt="logo"> </a> </div>
          <!-- /.logo -->
          <!-- ============================================================= LOGO : END ============================================================= --> </div>
        <!-- /.logo-holder -->
          ...
          <div class="navbar-collapse collapse" id="mc-horizontal-menu-collapse">
            <div class="nav-outer">
              <ul class="nav navbar-nav">
                <li class="active dropdown yamm-fw"> <a href="home.html" data-hover="dropdown" class="dropdown-toggle" data-toggle="dropdown">{{ (session()->get('language') == 'hindi')?'घर':'Home'}}</a> </li>
            ...
            @endphp
               ...
               @foreach ($categories as $category)
                 <li class="dropdown yamm mega-menu"> <a href="" data-hover="dropdown" class="dropdown-toggle" data-toggle="dropdown">{{ (session()->get('language') == 'hindi' )? $category->c_name_hin:$category->c_name_eng  }}</a>
                   <ul class="dropdown-menu container">
                     <li>
                       <div class="yamm-content ">
                         <div class="row">
            @php
            $subcategories = App\Models\Subcategory::where('c_id', $category->id)->orderBy('sc_name_eng', 'ASC')->get();
            @endphp
                            @foreach ($subcategories as $sub_cat)
                           <div class="col-xs-12 col-sm-6 col-md-2 col-menu">
                            <a href="{{ url('/subcategory/product/'.$sub_cat->id.'/'.$sub_cat->sc_slug_eng) }}">
                              <h2 class="title">{{ (session()->get('language') == 'hindi')? $sub_cat->sc_name_hin:$sub_cat->sc_name_eng }}</h2>
                            </a>
            @php
            $subsubcategories = App\Models\Subsubcategory::where('sc_id', $sub_cat->id)->orderBy('ssc_name_eng', 'ASC')->get();
            @endphp

                             @foreach ($subsubcategories  as $subsub)
                                <ul class="links">
                                  <li><a href="{{ url('/subsubcategory/product/'.$subsub->id.'/'.$subsub->ssc_slug_eng) }}">{{ (session()->get('language')== 'hindi')?$subsub->ssc_name_hin:$subsub->ssc_name_eng }}</a></li>
                                </ul>
                             @endforeach
                           </div>
                           <!-- /.col -->
                           ...
                           <div class="col-xs-12 col-sm-6 col-md-4 col-menu banner-image"> <img class="img-responsive" src="{{ asset('frontend_assets') }}/assets/images/banners/top-menu-banner.jpg" alt=""> </div>
                           <!-- /.yamm-content -->
                         </div>
                       </div>
                     </li>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
            <div class="col-md-4">
                <div class="card">
                    <img src=" " class="card-image-top" id="pimage" style="width:200px;height:200px">
                </div>
            </div>

            <div class="col-md-4">
                <ul class="list-group">
                    <li class="list-group-item ">Product Price: $<strong class="text-primary" id="pprice" ></strong> $<del class="text-primary" id="oldprice" ></del> </li>
                    <li class="list-group-item">Product Code: <strong class="text-primary" id="pcode" ></strong></li>
                    <li class="list-group-item">Category: <strong class="text-primary" id="pcat" ></strong> </li>
                    <li class="list-group-item">Brand: <strong class="text-primary" id="pbrand" ></strong> </li>
                    <li class="list-group-item">Stock: <strong class="text-primary" id="pstock" ></strong> </li>
                  </ul>
            </div>
        ...
        function miniCart(){
            ...
            $.ajax({
                ...
                duccess:function(response){
                    ...
                    <div class="col-xs-4">
                      <div class="image"> <a href="detail.html"><img src="{{ asset('frontend_assets') }}/assets/images/cart.jpg" alt=""></a> </div>
                    </div>
                    <div class="col-xs-7">
